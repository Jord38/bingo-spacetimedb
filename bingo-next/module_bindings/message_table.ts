...
export class MessageTableHandle {
  ...
  /**
   * Access to the `messageId` unique index on the table `message`,
   * which allows point queries on the field of the same name
   * via the [`MessageMessageIdUnique.find`] method.
   *
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.message.messageId().find(...)`.
   *
   * Get a handle on the `messageId` unique index on the table `message`.
   */
  messageId = {
