...
export class UserTableHandle {
  ...
  /**
   * Access to the `steamId` unique index on the table `user`,
   * which allows point queries on the field of the same name
   * via the [`UserSteamIdUnique.find`] method.
   *
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.user.steamId().find(...)`.
   *
   * Get a handle on the `steamId` unique index on the table `user`.
   */
  steamId = {
    // Find the subscribed row whose `steamId` column value is equal to `col_val`,
    // if such a row is present in the client cache.
    find: (col_val: string): User | undefined => {
      for (let row of this.tableCache.iter()) {
        if (deepEqual(row.steamId, col_val)) {
          return row;
        }
      }
    },
